          ...
          <DialogContent className="sm:max-w-lg bg-background/95 backdrop-blur-2xl border-border/40 p-0 flex flex-col max-h-[92vh]">
            ...
            {mealPlan && selectedMeal === null && (
              <div className="border-b border-border/30 bg-muted/10">
                <div className="px-4 pt-3 pb-2 flex flex-wrap gap-1.5 justify-start sm:justify-center">
                {mealPlan.days.map((d, idx) => (
                  <button
                    key={d.day}
                    onClick={() => setSelectedDay(idx)}
                    className={`flex-1 min-w-[3rem] px-2 py-1.5 rounded-xl text-xs font-bold transition-all duration-150 text-center ${
                      idx === selectedDay
